...
const TambahPengguna: React.FC<TambahPenggunaProps> = ({ isOpen, onClose }) => {
    interface Role {
        id: string;
        name: string;
    }
    ...
    const inputStyles = 'block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6';
    ...
    const handleSubmit = async (formData: FormData) => {
        try {
            ...
            const response = await fetch(`/api/akses/pengguna`, {
                method: 'POST',
                headers: {
    ...
    return (
        ...
        >
            ...
            <div className="mb-4 text-sm">
                ...
                <AsyncSelectComponent placeholder="Pilih Peran" optionLabel="name" optionValue="id" value={role} onChange={(selectedOption) => setRole(selectedOption as Role)} loadOptions={async () => {
