  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {canViewTop3Services && (
            <div className="bg-white p-4 rounded-lg shadow-sm border">
              <div className="font-semibold text-gray-800 p-3 mb-3">3 Layanan Teratas</div>
              <TopServiceType />
            </div>
          )}
        </div>
      ...
      <div className="space-y-4">
        {(canViewOperationalStatus && session?.user?.branch?.type === 'BRANCH') && (
          <OperationalStatus />
        )}

        {(canViewDailyCabinChecks && session?.user?.branch?.type === 'BRANCH') && (
          <DailyChecklist />
        )}
      ...
      {canViewTopVisitedBranches && (
        <div className="lg:col-span-4">
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            <div className="font-semibold text-gray-800 mb-3">Cabang Teratas</div>
