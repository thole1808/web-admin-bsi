...
interface Stat {
  status: 'ALL' | 'COMPLETED' | 'EXPIRE' | 'ACTIVE';
  current: number;
  previous: number;
        ...
        .map((stat, index) => (
          ...
          >
            ...
            <div className="ml-4 flex-1">
              <h4 className="text-sm font-medium text-gray-500 mb-1">{getStatusLabel(stat.status)}</h4>
              <p className="text-2xl font-bold text-gray-800 leading-tight">{stat.current}</p>
...
const getStatusLabel = (status: string) => {
  switch (status) {
    case 'EXPIRE': return 'Missed';
    case 'COMPLETED': return 'Completed';
    case 'ALL': return 'Total Visitors';
...
const getStatusIcon = (status: string) => {
  switch (status) {
    case 'EXPIRE': return <FaThumbsDown className="text-red-500 text-xl" />;
    case 'COMPLETED': return <FaThumbsUp className="text-green-500 text-xl" />;
    case 'ACTIVE': return <FaLandMineOn className="text-orange-500 text-xl" />;
    case 'ALL': return <FaUser className="text-blue-500 text-xl" />;
    default: return null;
  }
};
...
const getTrendIcon = (trend: 'UP' | 'DOWN' | 'STABLE') => {
  switch (trend) {
    case 'UP': return <FaArrowTrendUp className="text-green-500 mr-2" />;
    case 'DOWN': return <FaArrowTrendDown className="text-red-500 mr-2" />;
