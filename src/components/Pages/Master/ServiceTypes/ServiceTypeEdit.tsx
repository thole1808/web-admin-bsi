...
const ServiceTypeEdit: React.FC<ServiceTypeEditProps> = ({ isOpen, onClose, data, layanan }) => {
    const [formData, setFormData] = useState<FormData>({
        ...
        parentId: layanan?.find((item: any) => item.id === data.parentId) ? { value: data.parentId, label: layanan?.find((item: any) => item.id === data.parentId).name } : null,
    });
    ...
    const handleSubmit = async (formData: FormData) => {
        try {
            ...
            if (response.status === 422) {
                ...
                toast.success('Layanan successfully updated');
                onClose();
            } else {
